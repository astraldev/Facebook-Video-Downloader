...
mcp("𝘼𝙪𝙩𝙝𝙤𝙧   -","★[ᴄᴏᴅᴇᴘʟᴀʏ403]★", color="cg" )
mcp("𝙁𝙖𝙘𝙚𝙗𝙤𝙤𝙠 -","https ://www.facebook.com/Master404/", color="cg")
mcp("𝙂𝙞𝙩𝙝𝙪𝙗   - ","https://github.com/Codeplay403/", color="cg")
    ...
    try:
        ...
        if len(list) == 2:
            ...
            if 1 in list and 2 in list:
                Print("\nOops! The video is not available in HD quality. Would you like to download it? ('Y' or 'N'): ", color='m')
                _input_2 = str(input()).upper()
                if _input_2 == 'Y':
                    download_video("SD")
                if _input_2 == 'N':
                    exit()
...
def download_video(quality):
    ...
    with open(filename + '.mp4', 'wb') as f:
        for data in file_size_request.iter_content(block_size):
            ...
            f.write(data)
    t.close()
    Print("\nVideo downloaded successfully.", color="g")


try:
    while True:
        Print("\nEnter the URL of Facebook video: ", color="b")
        url = input()
        ...
        if x:
            ...
            Print("\nNot related with Facebook domain.", color="r" )
            exit()
        ...
        main()
        Print ("\nDo you want download another video? (Y or N): ", color="b")
        again = input().upper()
        if again == str("Y"):
